...
try:
    from modules_l1a_SWIR.l1agen_SWIR_Image_Data_TEC_Metadata_Matcher import LocateFiles
    from modules_l1a_SWIR.l1agen_SWIR_Img_Processor import SWIR_img_processor
    from modules_l1a_SWIR import l1agen_Constants as SWIR_constants
except ImportError:
...
class NC_Processor():
    ...
    def __init__(self, fname, out_dir, start_time, end_time, filt_pos, int_time, cfc_capture_id, tec_reading, processing_level, include_er2_nav=False):
        ...
        self.int_time           = int_time / SWIR_constants.INT_TIME_US_TO_MS  # convert to milliseconds ONLY FOR USE IN NetCDF4 FILE
    ...
    def init_nc(self):
        ...
        nc = Dataset(self.fpath, mode = 'w', format = SWIR_constants.NC_FORMAT)
    ...
    def set_global_attrs(self, nc):
        ...
        nc.source               = SWIR_constants.NC_SOURCE
        nc.HARP2_L1AGen_version = SWIR_constants.VERSION  # Version adapted from HARP2_L1AGen
        nc.SWIR_L1AGen_version  = SWIR_constants.SWIR_L1A_GEN_VERSION
    ...
    def init_img_data_grp(self, nc):
        ...
        img_grp.createDimension("sensors", SWIR_constants.IMG_SENSORS)
        img_grp.createDimension("lines", SWIR_constants.IMG_LINES)            # rows
        img_grp.createDimension("pixels", SWIR_constants.IMG_PIXELS)           # columns for SWIR
        img_grp.createDimension("number_filters", SWIR_constants.SWIR_NUM_FILTERS)

        #-- create unified variables with filter position dimensions
        # Image data with unified frames: sensor1[frame, line, pixel]
        img_grp.createVariable(
            "sensor1",
            datatype = "u2",
            dimensions = ("frames", "lines", "pixels"),
            fill_value = SWIR_constants.NC_SENSOR_FILL_U2,
        )
        
        # Frame and timestamps with filter position dimension
        img_grp.createVariable("frame_ID", datatype = "i4", dimensions = ("frames", "sensors"), fill_value = SWIR_constants.NC_FILL_VALUE)
        img_grp.createVariable("time_stamp", datatype = "i8", dimensions = ("frames", "sensors"), fill_value = SWIR_constants.NC_FILL_VALUE)
        img_grp.createVariable("JD", datatype = "f8", dimensions = ("frames", "sensors"), fill_value = SWIR_constants.NC_FILL_VALUE)
        img_grp.createVariable("seconds_of_day", datatype = "f8", dimensions = ("frames", "sensors"), fill_value = SWIR_constants.NC_FILL_VALUE)
        img_grp.createVariable("acquisition_type", datatype = "i4", dimensions = ("frames", "sensors"), fill_value = SWIR_constants.NC_FILL_VALUE)
        img_grp.createVariable("image_type", datatype = "i4", dimensions = ("frames", "sensors"), fill_value = SWIR_constants.NC_FILL_VALUE)
        img_grp.createVariable("binning_cross_track", datatype = "i4", dimensions = ("frames", "sensors"), fill_value = SWIR_constants.NC_FILL_VALUE)
        
        # Sensor information with filter position dimension
        img_grp.createVariable("sensor_temperature", datatype = "f4", dimensions = ("frames", "sensors"), fill_value = SWIR_constants.NC_FILL_VALUE)
        img_grp.createVariable("integration_time", datatype = "f4", dimensions = ("frames", "sensors"), fill_value = SWIR_constants.NC_FILL_VALUE)
        

        #-- define units, long_name, valid_min, and valid_max for each variable as applicable
        filter_descriptions = SWIR_constants.SWIR_FILTER_DESCRIPTIONS_LONG
        filter_wavelengths = SWIR_constants.SWIR_FILTER_WAVELENGTHS_UM  # wavelengths in micrometers
        
        # Unified sensor variable
        img_grp["sensor1"].units = "counts"
        img_grp["sensor1"].long_name = "Image read from SWIR Sensor counts"
        img_grp["sensor1"].sensor_bitdepth = SWIR_constants.SENSOR_BITDEPTH
        img_grp["sensor1"].valid_min = SWIR_constants.SENSOR_VALID_MIN
        img_grp["sensor1"].valid_max = SWIR_constants.SENSOR_VALID_MAX
        # CF-convention friendly scaling metadata (no-op unless changed later)
        img_grp["sensor1"].scale_factor = SWIR_constants.SENSOR_SCALE_FACTOR
        img_grp["sensor1"].add_offset = SWIR_constants.SENSOR_ADD_OFFSET
        img_grp["sensor1"].comment = "Access data by frame: sensor1[frame, line, pixel]"
        img_grp["sensor1"].dimensions_info = SWIR_constants.IMG_DIMENSIONS_INFO
        ...
        img_grp["sensor_temperature"].valid_min = SWIR_constants.SENSOR_TEMPERATURE_VALID_MIN
        img_grp["sensor_temperature"].valid_max = SWIR_constants.SENSOR_TEMPERATURE_VALID_MAX
        ...
        img_grp["integration_time"].valid_min = SWIR_constants.INTEGRATION_TIME_VALID_MIN
        img_grp["integration_time"].valid_max = SWIR_constants.INTEGRATION_TIME_VALID_MAX
        ...
        img_grp.createVariable("number_filters", datatype = "i4", dimensions = ("number_filters",), fill_value = SWIR_constants.NC_FILL_VALUE)
        img_grp["number_filters"][:] = SWIR_constants.SWIR_FILTER_POSITIONS
        img_grp["number_filters"].long_name = "Filter numbers (0-4)"

        # Filter wheel position per frame
        img_grp.createVariable("filter_wheel_position", datatype = "i4", dimensions = ("frames",), fill_value = SWIR_constants.NC_FILL_VALUE)
    ...
    def init_nav_data_grp(self, nc):
        ...
        nav_grp.createDimension("quaternion_elements", SWIR_constants.NAV_QUATERNION_ELEMENTS)
        nav_grp.createDimension("vector_elements", SWIR_constants.NAV_VECTOR_ELEMENTS)
        ...
        nav_grp["att_time"].valid_min  = SWIR_constants.NAV_ATT_TIME_VALID_MIN
        nav_grp["att_time"].valid_max  = SWIR_constants.NAV_ATT_TIME_VALID_MAX
        ...
        nav_grp["att_quat"].valid_min  = SWIR_constants.NAV_ATT_QUAT_VALID_MIN
        nav_grp["att_quat"].valid_max  = SWIR_constants.NAV_ATT_QUAT_VALID_MAX
        nav_grp["att_rate"].units      = "radians/second"
        nav_grp["att_rate"].long_name  = "Attitude angular rates in spacecraft frame"
        nav_grp["att_rate"].valid_min  = SWIR_constants.NAV_ATT_RATE_VALID_MIN
        nav_grp["att_rate"].valid_max  = SWIR_constants.NAV_ATT_RATE_VALID_MAX
        nav_grp["att_euler"].units     = "degrees"
        nav_grp["att_euler"].long_name = "Euler angles (roll, pitch, yaw) in spacecraft frame"
        nav_grp["att_euler"].valid_min = SWIR_constants.NAV_ATT_EULER_VALID_MIN
        nav_grp["att_euler"].valid_max = SWIR_constants.NAV_ATT_EULER_VALID_MAX
        ...
        nav_grp["orb_pos"].valid_min   = SWIR_constants.NAV_ORB_POS_VALID_MIN
        nav_grp["orb_pos"].valid_max   = SWIR_constants.NAV_ORB_POS_VALID_MAX
        nav_grp["orb_vel"].units       = "meters/second"
        nav_grp["orb_vel"].long_name   = "Spacecraft velocity vector in ECEF (ECR) frame"
        nav_grp["orb_vel"].valid_min   = SWIR_constants.NAV_ORB_VEL_VALID_MIN
        nav_grp["orb_vel"].valid_max   = SWIR_constants.NAV_ORB_VEL_VALID_MAX
        nav_grp["orb_lon"].units       = "degrees"
        nav_grp["orb_lon"].long_name   = "Orbit longitude (degrees East)"
        nav_grp["orb_lon"].valid_min   = SWIR_constants.NAV_ORB_LON_VALID_MIN
        nav_grp["orb_lon"].valid_max   = SWIR_constants.NAV_ORB_LON_VALID_MAX
        nav_grp["orb_lat"].units       = "degrees"
        nav_grp["orb_lat"].long_name   = "Orbit latitude (degrees North)"
        nav_grp["orb_lat"].valid_min   = SWIR_constants.NAV_ORB_LAT_VALID_MIN
        nav_grp["orb_lat"].valid_max   = SWIR_constants.NAV_ORB_LAT_VALID_MAX
        nav_grp["orb_alt"].units       = "meters"
        nav_grp["orb_alt"].long_name   = "Orbit altitutde"
        nav_grp["orb_alt"].valid_min   = SWIR_constants.NAV_ORB_ALT_VALID_MIN
        nav_grp["orb_alt"].valid_max   = SWIR_constants.NAV_ORB_ALT_VALID_MAX
    ...
    def init_er2_nav_data_grp(self, nc):
        ...
        nc.createDimension("er2_vector_elements", SWIR_constants.ER2_VECTOR_ELEMENTS)
        ...
        er2_grp["orb_time"].valid_min   = SWIR_constants.ER2_ORB_TIME_VALID_MIN
        er2_grp["orb_time"].valid_max   = SWIR_constants.ER2_ORB_TIME_VALID_MAX
        er2_grp["orb_lon"].units        = "degrees"
        er2_grp["orb_lon"].long_name    = "ER2 longitude (degrees East)"
        er2_grp["orb_lon"].valid_min    = SWIR_constants.ER2_ORB_LON_VALID_MIN
        er2_grp["orb_lon"].valid_max    = SWIR_constants.ER2_ORB_LON_VALID_MAX
        er2_grp["orb_lat"].units        = "degrees"
        er2_grp["orb_lat"].long_name    = "ER2 latitude (degrees North)"
        er2_grp["orb_lat"].valid_min    = SWIR_constants.ER2_ORB_LAT_VALID_MIN
        er2_grp["orb_lat"].valid_max    = SWIR_constants.ER2_ORB_LAT_VALID_MAX
        er2_grp["orb_alt"].units        = "meters"
        er2_grp["orb_alt"].long_name    = "ER2 altitude"
        er2_grp["orb_alt"].valid_min    = SWIR_constants.ER2_ORB_ALT_VALID_MIN
        er2_grp["orb_alt"].valid_max    = SWIR_constants.ER2_ORB_ALT_VALID_MAX
        er2_grp["att_euler"].units      = "degrees"
        er2_grp["att_euler"].long_name  = "ER2 Euler angles (roll, pitch, yaw) in spacecraft frame"
        er2_grp["att_euler"].valid_min  = SWIR_constants.ER2_ATT_EULER_VALID_MIN
        er2_grp["att_euler"].valid_max  = SWIR_constants.ER2_ATT_EULER_VALID_MAX
    ...
    def write_img_to_l1a(self, img, filter_idx, iframe, cfc_capture_id, tec_temp=None, int_time_ms=None):
        ...
        self.nc_obj["image_data"]["binning_cross_track"][gframe, 0] = SWIR_constants.DEFAULT_BINNING_CROSS_TRACK  # no binning
        self.nc_obj["image_data"]["acquisition_type"][gframe, 0] = SWIR_constants.DEFAULT_ACQUISITION_TYPE  # Direct (SPW)
        self.nc_obj["image_data"]["image_type"][gframe, 0] = SWIR_constants.DEFAULT_IMAGE_TYPE  # Science
...
def build_obpg_filename(
    mission,
    instrument,
    timestamp,
    level,
    version=SWIR_constants.OBPG_DEFAULT_VERSION,
    resolution=None,
    product=None,
    revision=None,
):
...
def copy_imu_to_l1a(nc4, ah2_imu_file, start_sec, end_sec, verbose=False):
    ...
    pad = SWIR_constants.NAV_PAD_SECONDS  # include padding for navigation data
...
def write_er2_imu_given_time(
    nc4,
    csv_file,
    xml_file,
    start_sec,
    end_sec,
    pad_sec=SWIR_constants.ER2_PAD_SECONDS,
    verbose=False,
):
    """
...
def main():
    ...
    if summary['total_images'] > 0:
        ...
        if earliest_time:
            time_str = earliest_time.strftime(SWIR_constants.OBPG_TIME_FORMAT)
        else:
            time_str = "unknown"
    else:
        start_time = "unknown"
    ...
    fname = build_obpg_filename(
        mission=SWIR_constants.OBPG_DEFAULT_MISSION,
        instrument=SWIR_constants.OBPG_DEFAULT_INSTRUMENT,
        timestamp=time_str,
    ...
    if summary['total_images'] > 0:
        representative_tec = int(np.mean(swir_processor.matched_data['tec_temps'])) if len(swir_processor.matched_data['tec_temps']) > 0 else SWIR_constants.DEFAULT_REPRESENTATIVE_TEC
        representative_int_time = int(np.mean(swir_processor.matched_data['integration_times']) * SWIR_constants.INT_TIME_US_TO_MS) if len(swir_processor.matched_data['integration_times']) > 0 else SWIR_constants.DEFAULT_REPRESENTATIVE_INT_TIME_US  # convert to microseconds
        representative_cfc_id = int(swir_processor.matched_data['cfc_ids'][0]) if len(swir_processor.matched_data['cfc_ids']) > 0 else SWIR_constants.DEFAULT_REPRESENTATIVE_CFC_ID
    else:
        representative_tec = SWIR_constants.DEFAULT_REPRESENTATIVE_TEC
        representative_int_time = SWIR_constants.DEFAULT_REPRESENTATIVE_INT_TIME_US
        representative_cfc_id = SWIR_constants.DEFAULT_REPRESENTATIVE_CFC_ID
    ...
    if include_er2_nav and er2_infile and er2_xmlfile:
        if os.path.exists(er2_infile) and os.path.exists(er2_xmlfile):
            ...
            if earliest_time and latest_time:
                ...
                nc_for_er2 = write_er2_imu_given_time(
                    nc_for_er2,
                    er2_infile,
                    er2_xmlfile,
                    start_sec,
    ...
    filter_descriptions = SWIR_constants.SWIR_FILTER_LABELS
